...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        ens.nom_ens AS professeur,
        s.type_seance,
        sal.nom_salle,
        f.nom_filiere,
        c.nom_classe,
        CASE s.type_seance
            WHEN 'CM' THEN 'Cours Magistral'
            WHEN 'TD' THEN 'Travaux Dirigés'
            WHEN 'TP' THEN 'Travaux Pratiques'
            WHEN 'EXAMEN' THEN 'Examen'
            WHEN 'RATTRAPAGE' THEN 'Rattrapage'
            ELSE s.type_seance
        END AS type_seance_complet,
        CASE s.type_seance
            WHEN 'CM' THEN 'blue'
            WHEN 'TD' THEN 'green'
            WHEN 'TP' THEN 'orange'
            WHEN 'EXAMEN' THEN 'red'
            WHEN 'RATTRAPAGE' THEN 'purple'
            ELSE 'gray'
        END AS couleur_type

        FROM seance s
        INNER JOIN ues ue ON s.id_ue = ue.id_ue
        INNER JOIN enseignant ens ON s.id_enseignant = ens.id_enseignant
        INNER JOIN classe c ON ue.id_classe = c.id_classe
        INNER JOIN filiere f ON c.id_filiere = f.id_filiere
        LEFT JOIN salle sal ON s.id_salle = sal.id_salle

        WHERE f.nom_filiere = :nom_filiere 
        AND c.nom_classe = :nom_classe
        AND s.statut IN ('PLANIFIEE', 'CONFIRMEE', 'TERMINEE')
...
<body>
    <div class="container">
        ...
        <?php else: ?>
            <div class="message error">
                Aucune séance programmée pour cette classe.
            </div>
            <div class="actions">
